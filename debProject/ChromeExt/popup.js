const proxyServerUrl = 'https://your-app-name.herokuapp.com/'; // Replace this with your Heroku proxy server URL
function fetchNews(url, sourceName) {
    // Use the proxy server to fetch the news data
    const proxiedUrl = proxyServerUrl + encodeURIComponent(url);
    fetch(proxiedUrl, {
            method: 'GET',
            mode: 'cors',
                ...
                .catch(error => {
                    if (error.name === 'TypeError') {
                        ...
                        if (numItemsAdded === 0) {
                            ...
                            noItems.textContent = 'No results found';
                            sourceList.appendChild(noItems);
                        }
                        li.innerHTML = `<div class="news-item"><a href="${linkText}" target="_blank">
                      ${title.textContent}</a><p>${description.textContent}</p></div>`;
                        sourceList.appendChild(li);
